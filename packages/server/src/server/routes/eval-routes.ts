...
import { getSession, updateSessionProgress } from '../../../../../src/db/sessions.ts';
...
export const routes: RouteHandler[] = [
  {
    ...
    handler: async (req) => {
      ...
      return json({ sessionId, checkpoints: [], note: 'Use /api/memori/checkpoints for full checkpoint listing' });
    },
  },
  {
    method: 'GET',
  ...
  },
  {
    ...
    handler: async (_req, path) => {
      ...
      const resumePrompt = buildResumePrompt(restored) + (restored.toolCallHistory.length > 0 ? `\n\n## Tool History\n${restored.toolCallHistory.map((t) => `- ${t.toolName}`).join('\n')}` : '');
      ...
      try {
        ...
        await db.run(`INSERT INTO session_messages (role, content, token_count, created_at) VALUES (?, ?, ?, ?)`, ['system', resumePrompt, null, checkpoint.timestamp]);
        for (const message of checkpoint.conversation.messages) {
          await db.run(`INSERT INTO session_messages (role, content, token_count, created_at) VALUES (?, ?, ?, ?)`, [message.role, message.content, null, message.timestamp ?? checkpoint.timestamp]);
        }
        await db.exec('COMMIT');
      } catch (e) { await db.exec('ROLLBACK').catch(() => {}); throw e; }
      await updateSessionProgress(checkpoint.sessionId, checkpoint.turnNumber, checkpoint.timestamp, checkpoint.agentId);
      return json({ success: true, sessionId: checkpoint.sessionId, checkpointId, turnNumber: checkpoint.turnNumber });
    },
  },
  {
    method: 'GET',
  ...
  },
  {
    ...
    handler: async (req) => {
      ...
      const body = await req.json() as { suiteId: string; agentId?: string; provider?: string; baselineId?: string; timeout?: number; };
      ...
      const run = await runSuite(suite, { provider, model: config.providers[provider]?.model ?? 'unknown' } as never);
      return json(run, 201);
    },
  },
  {
  ...
  },
  {
    ...
    handler: async (req) => {
      const body = await req.json() as { query: string; retrievedDocs?: string[]; expectedDoc?: string; };
      ...
      return json({ query: body.query, retrievedCount: retrieved.length, hitAt1: hit, recall: body.expectedDoc ? (hit ? 1 : 0) : null, mrr: hit ? 1 : 0 });
    },
  },
  {
    method: 'GET',
    ...
    handler: async () => {
      const config = await loadConfig();
      return json({ providers: Object.keys(config.providers ?? {}).map((k) => ({ kind: k, model: config.providers?.[k as keyof typeof config.providers]?.model ?? 'unknown', hasKey: !!config.providers?.[k as keyof typeof config.providers]?.apiKey })), recommendation: 'Analysis from quartermaster integration pending' });
    },
  },
  {
    method: 'GET',
    ...
    handler: async (req) => {
      ...
      return json({ traces: [], otelEnabled: !!Deno.env.get('OTEL_EXPORTER_OTLP_ENDPOINT'), langfuseEnabled: !!Deno.env.get('LANGFUSE_PUBLIC_KEY') });
    },
  },
  {
    method: 'GET',
    ...
    handler: async () => {
      ...
      } catch { return json({ suites: [], comparisons: [] }); }
    },
  },
  {
    method: 'GET',
    ...
    handler: async () => {
      return json({ presets: [
        { name: 'code-generation', tasks: ['write function', 'fix bug', 'refactor class'], scoring: 'pass@1' },
        { name: 'code-exploration', tasks: ['find symbol', 'trace dependency', 'explain architecture'], scoring: 'accuracy' },
        { name: 'qa-bench', tasks: ['answer question', 'cite sources', 'explain concept'], scoring: 'f1' },
        { name: 'security-audit', tasks: ['scan prompt', 'check hygiene', 'validate policy'], scoring: 'precision@k' },
      ], recentRuns: [] });
    },
  },
  {
  ...
  },
  {
    ...
    handler: async (req) => {
      ...
      const conn = connectPkm(body.kind as 'obsidian' | 'logseq' | 'notion' | 'roam', body.path, body.name || body.path);
  ...
  },
  {
    ...
    handler: async (req) => {
      ...
      try { return json(await syncPkm(body.id)); } catch (e) { return err((e as Error).message, 400); }
    },
  },
  {
    method: 'GET',
    ...
    handler: async (req) => {
      const url = new URL(req.url);
      const { listTerms, getCategories } = await import('../../../../../src/memory/glossary.ts');
      const category = url.searchParams.get('category');
      const [terms, categories] = await Promise.all([listTerms(category || undefined), getCategories()]);
  ...
  },
  {
    ...
    pattern: /^\/api\/glossary$/,
    handler: async (req) => {
      const body = await req.json() as { name: string; definition: string; category?: string; aliases?: string[]; };
      if (!body.name || !body.definition) return err('name and definition required', 400);
      const { defineTerm } = await import('../../../../../src/memory/glossary.ts');
  ...
  },
  {
    ...
    handler: async (req) => {
      ...
      return json({ templates: listPromptTemplates(), runs: listPromptRuns(templateId || undefined) });
    },
  },
  {
    method: 'POST',
    pattern: /^\/api\/prompts$/,
    handler: async (req) => {
      const body = await req.json() as { name: string; content?: string; tags?: string[]; model?: string; input?: string; output?: string; score?: number; };
      if (!body.name) return err('name required', 400);
      const { createPromptTemplate, recordPromptRun } = await import('../../prompt-lab.ts');
      if (body.content) { const tpl = createPromptTemplate(body.name, body.content, body.tags); return json(tpl, 201); }
      if (body.input && body.output) { const run = recordPromptRun(body.name, body.model || 'default', body.input, body.output, body.score); return json(run, 201); }
  ...
  },
  {
    ...
    handler: async () => {
      return json({ stages: ['chunk', 'embed', 'index', 'backfill'], backends: ['lancedb', 'chroma', 'pinecone'], active: false, config: { chunkSize: 512, chunkOverlap: 64, batchSize: 32 } });
    },
  },
];
