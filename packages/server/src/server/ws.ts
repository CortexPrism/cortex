...
export async function handleWebSocket(req: Request): Promise<Response> {
  ...
  async function processChatMessage(
    ...
    try {
      ...
      let providerKind: import('../../../../src/config/config.ts').ProviderKind;
      let model: string;
      ...
      if (files?.length) {
        const { extractPdfText } = await import('../../../../src/utils/pdf.ts');
        for (const file of files) {
      ...
      {
        const { getLoadedPluginSummaries } = await import('../../../../src/plugins/loader.ts');
      ...
      try {
        const { getStoredSideEffect } = await import('../../../../src/pipeline/manager.ts');
