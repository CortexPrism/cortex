...
import { getSessionEvents } from '../../../../src/db/lens.ts';
import { getLensDb, type InValue } from '../../../../src/db/client.ts';
import { getJob, listJobRuns, listJobs } from '../../../../src/scheduler/scheduler.ts';
import { retrieve, writeEpisodic } from '../../../../src/memory/store.ts';
import {
...
import { handleI18nApi } from '../../../../src/i18n/api.ts';
...
import { type findDenoProcesses, resolveHomeDir } from '../../../../src/utils/platform.ts';
...
import {
  ...
  getRuntimeStatus,
  getService,
  type isServiceManagerActive,
  listServices,
  registerService,
  startService,
  stopService,
  updateService,
} from '../../../../src/services/manager.ts';
...
export async function handleApi(req: Request): Promise<Response | null> {
  ...
  if (req.method === 'POST' && path === '/api/onboarding/complete') {
    ...
    cfg.onboarding = {
      ...
      version: (await import('../../../../src/config/version.ts')).ONBOARDING_VERSION,
      skippedSteps: [],
    };
    await saveConfig(config);
    await runMigrations();
  ...
  if (req.method === 'GET' && path === '/api/system') {
    ...
    const { getVersion } = await import('../../../../src/config/version.ts');
    return json({
  ...
  if (req.method === 'POST' && path === '/api/jobs/recover') {
    const { recoverStaleJobs } = await import('../../../../src/scheduler/scheduler.ts');
  ...
  if (req.method === 'GET' && path === '/api/system/diagnostics') {
    ...
    try {
      const { getAvailableRuntime } = await import('../../../../src/sandbox/executor.ts');
  ...
  if (req.method === 'GET' && complianceSessionMatch) {
    const { getSessionCompliance } = await import('../../../../src/security/compliance.ts');
  ...
  if (req.method === 'GET' && path === '/api/compliance/risk') {
    const { getComplianceByRisk } = await import('../../../../src/security/compliance.ts');
  ...
  if (req.method === 'GET' && path === '/api/compliance/export') {
    const { exportComplianceReport } = await import('../../../../src/security/compliance.ts');
  ...
  if (req.method === 'POST' && path === '/api/compliance/retention') {
    const { enforceRetention } = await import('../../../../src/security/compliance.ts');
  ...
  if (req.method === 'POST' && path === '/api/metacognition/test') {
    ...
    const { assessTask } = await import('../../../../src/agent/metacog.ts');
  ...
  if (req.method === 'PUT' && path === '/api/config') {
    ...
    if (body.logging) {
      ...
      configureLogger({
        level: (lc.level ?? 'error') as import('../../../../src/utils/logger.ts').LogLevel,
  ...
  if (req.method === 'GET' && path === '/api/analytics') {
    ...
    const coreDb = await (await import('../../../../src/db/client.ts')).getCoreDb();
  ...
  if (req.method === 'GET' && path === '/api/hooks') {
    const { listHooks } = await import('../../../../src/pipeline/manager.ts');
    return json(
      listHooks().map((r) => ({
        name: r.hook.name,
  ...
  if (req.method === 'POST' && path === '/api/hooks/init') {
    const { registerBuiltinHooks } = await import('../../../../src/pipeline/builtin.ts');
    const { getHookCount } = await import('../../../../src/pipeline/manager.ts');
  ...
  if (req.method === 'POST' && hookDisableMatch) {
    const { unregisterHook } = await import('../../../../src/pipeline/manager.ts');
  ...
  if (req.method === 'DELETE' && jobDeleteStatusMatch) {
    ...
    await deleteJobsByStatus(
      jobDeleteStatusMatch[1] as import('../../../../src/scheduler/scheduler.ts').JobStatus,
    );
    return json({ ok: true });
  }
  ...
  if (req.method === 'GET' && path === '/api/agents/current') {
    const { getDefaultAgent } = await import('../../../../src/agent/manager.ts');
  ...
  if (req.method === 'GET' && path === '/api/agents/sub-types') {
    const { listSubAgentTypes } = await import('../../../../src/agent/sub-agent-types.ts');
  ...
  if (req.method === 'GET' && path === '/api/agents/builtin') {
    const { getBuiltinAgentDefs } = await import('../../../../src/agent/builtin-agents.ts');
  ...
  if (req.method === 'PUT' && subAgentTypesMatch) {
    ...
    const { SUB_AGENT_TYPES } = await import('../../../../src/agent/sub-agent-types.ts');
  ...
  if (req.method === 'GET' && agentIdentityMatch) {
    ...
    if (!agent) return notFound('Agent not found');
    const { loadAgentIdentity } = await import('../../../../src/agent/manager.ts');
  ...
  if (req.method === 'POST' && agentCloneMatch) {
    const { cloneAgent } = await import('../../../../src/agent/manager.ts');
  ...
  if (req.method === 'GET' && path === '/api/providers/comparison') {
    const config = await loadConfig();
    const { PROVIDER_DEFAULT_CONTEXT_WINDOWS } = await import('../../../../src/llm/router.ts');
  ...
  if (req.method === 'GET' && path === '/api/security/supervisor') {
    const { selectSupervisorModel } = await import('../../../../src/security/supervisor.ts');
    const config = await loadConfig();
  ...
  if (req.method === 'DELETE' && path === '/api/security/supervisor/cache') {
    const { clearDecisionCache } = await import('../../../../src/security/supervisor.ts');
    clearDecisionCache();
    return json({ ok: true });
  }

  if (req.method === 'GET' && path === '/api/security/supervisor/history') {
    const { getDecisionCacheEntries } = await import('../../../../src/security/supervisor.ts');
  ...
  if (req.method === 'GET' && path === '/api/workspace/agents') {
    ...
    const { listAgents } = await import('../../../../src/agent/manager.ts');
    const agents = await listAgents();
  ...
  if (req.method === 'POST' && mpPluginInstallMatch) {
    ...
    const { installFromMarketplace } = await import('../../../../src/plugins/install.ts');
    try {
  ...
  if (req.method === 'POST' && mpAgentImportMatch) {
    ...
    const { registerAgent } = await import('../../../../src/agent/manager.ts');
    try {
  ...
  if (req.method === 'POST' && path === '/api/cacp/links') {
    ...
    const { linkSessions } = await import('../../../../src/memory/cross-agent-context.ts');
  ...
  if (req.method === 'DELETE' && cacpLinkDel) {
    const { unlinkSessions } = await import('../../../../src/memory/cross-agent-context.ts');
  ...
  if (req.method === 'POST' && path === '/api/sandbox/snapshots') {
    ...
    const { captureEnvironmentSnapshot } = await import('../../../../src/sandbox/replication.ts');
    return json(
  ...
  if (req.method === 'GET' && path === '/api/sandbox/snapshots') {
    const sessionId = url.searchParams.get('sessionId') ?? undefined;
    const limit = Number(url.searchParams.get('limit') ?? 50);
    const { listEnvironmentSnapshots } = await import('../../../../src/sandbox/replication.ts');
  ...
  if (
    ...
    const { getEnvironmentSnapshot } = await import('../../../../src/sandbox/replication.ts');
    const snap = await getEnvironmentSnapshot(envSnapMatch[1]);
    if (!snap) return notFound('Snapshot not found');
    const { maskSensitiveEnv } = await import('../../../../src/sandbox/replication.ts');
  ...
  if (req.method === 'DELETE' && envSnapMatch) {
    const { deleteEnvironmentSnapshot } = await import('../../../../src/sandbox/replication.ts');
