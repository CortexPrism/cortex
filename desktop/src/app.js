...
function loadDashboard() {
  var url = 'http://localhost:' + state.serverPort;
  var frame = el('dashboard-frame');
...
function openDashboardPage(hash) {
  var frame = el('dashboard-frame');
  var url = 'http://localhost:' + state.serverPort + '/' + (hash || '');
  try {
...
function initTauri() {
  ...
  window.__TAURI__.event.listen('quick-ask', function() {
    openQuickAskModal();
  }).catch(function() {});

  window.__TAURI__.event.listen('server-status-changed', function() {
