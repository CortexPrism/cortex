...
var plTemplates = [], plRuns = [], plABTests = [], plCurrentId = null, plCurrentABId = null, plGeneratedPrompt = '', plGenTab = 'templates';
function switchPLTab(tab) { plGenTab = tab; document.querySelectorAll('.pl-tab').forEach(function(b){ b.style.borderBottomColor = 'transparent'; b.style.color = 'var(--text2)'; }); var tb = document.querySelector('.pl-tab[data-pltab="'+tab+'"]'); if(tb){ tb.style.borderBottomColor = 'var(--accent)'; tb.style.color = 'var(--accent)'; } document.getElementById('pl-tab-templates').style.display = tab==='templates'?'flex':'none'; document.getElementById('pl-tab-abtests').style.display = tab==='abtests'?'flex':'none'; document.getElementById('pl-tab-generator').style.display = tab==='generator'?'flex':'none'; if(tab==='abtests') loadABTests(); }
function loadPromptLab() { fetch(BASE+'/api/prompts').then(function(r){return r.json()}).then(function(data){ plTemplates=data.templates||[]; plRuns=data.runs||[]; plABTests=data.abTests||[]; renderPLStats(data.stats); renderPromptTemplates(); renderPromptRuns(); }).catch(function(){}); }
function renderPLStats(s) { var el=document.getElementById('pl-stats'); if(!el||!s)return; el.innerHTML=s.templateCount+' templates · '+s.runCount+' runs · '+s.abTestCount+' tests'; }
function renderPromptTemplates() { var el=document.getElementById('pl-templates'); if(!el)return; el.innerHTML=plTemplates.length?plTemplates.map(function(t){ return '<div class="card-sm" style="cursor:pointer;margin-bottom:4px;padding:8px 12px;'+(plCurrentId===t.id?'border-left:3px solid var(--accent);':'')+'" onclick="selectPromptTemplate(\\''+t.id+'\\')"><div style="font-weight:500;font-size:12px;">'+esc(t.name)+'</div><div style="font-size:10px;color:var(--text3);">v'+t.version+(t.variables&&t.variables.length?' · vars: '+t.variables.join(', '):'')+'</div><div style="font-size:9px;color:var(--text3);">'+(t.tags||[]).join(', ')+'</div></div>' }).join(''):'<div class="empty">No templates yet</div>'; }
function selectPromptTemplate(id) { plCurrentId=id; var t=plTemplates.find(function(x){return x.id===id}); if(!t)return; document.getElementById('pl-editor-title').textContent=t.name+' (v'+t.version+')'; document.getElementById('pl-editor-text').style.display='block'; document.getElementById('pl-editor-text').value=t.content; document.getElementById('pl-editor-actions').style.display='flex'; document.getElementById('pl-editor-btns').style.display='flex'; if(t.variables&&t.variables.length){ document.getElementById('pl-variables').style.display='block'; document.getElementById('pl-variables').textContent='Variables: {{'+t.variables.join('}}, {{')+'}}'; } else { document.getElementById('pl-variables').style.display='none'; } renderPromptTemplates(); renderPromptRuns(); }
function showPromptCreateModal() { var n=prompt('Template name:');if(!n)return;var c=prompt('Prompt content:');if(!c)return;fetch(BASE+'/api/prompts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:n,content:c})}).then(function(r){return r.json()}).then(function(){toast('Template created','success');loadPromptLab()}); }
function savePromptTemplate() { if(!plCurrentId)return;var c=document.getElementById('pl-editor-text').value;fetch(BASE+'/api/prompts/'+plCurrentId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({content:c})}).then(function(){toast('Saved','success');loadPromptLab()}); }
function deletePromptTemplate() { if(!plCurrentId)return;if(!confirm('Delete this template and all its runs?'))return;fetch(BASE+'/api/prompts/'+plCurrentId,{method:'DELETE'}).then(function(){ plCurrentId=null; document.getElementById('pl-editor-text').style.display='none'; document.getElementById('pl-editor-actions').style.display='none'; document.getElementById('pl-editor-btns').style.display='none'; document.getElementById('pl-editor-title').textContent='Select a template'; document.getElementById('pl-variables').style.display='none'; toast('Deleted','info');loadPromptLab(); }); }
function showTestRunModal() { if(!plCurrentId)return;document.getElementById('pl-testrun-modal').style.display='flex';document.getElementById('pl-run-input').focus(); }
function closeTestRunModal() { document.getElementById('pl-testrun-modal').style.display='none'; }
function recordTestRun() { if(!plCurrentId)return;var inp=document.getElementById('pl-run-input').value;var out=document.getElementById('pl-run-output').value;if(!inp||!out){toast('Input and output required','error');return;} var sc=parseFloat(document.getElementById('pl-run-score').value)||undefined;var lat=parseInt(document.getElementById('pl-run-latency').value)||undefined;var tok=parseInt(document.getElementById('pl-run-tokens').value)||undefined;var md=document.getElementById('pl-run-model').value||'default';fetch(BASE+'/api/prompts/runs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({templateId:plCurrentId,model:md,input:inp,output:out,score:sc,latencyMs:lat,tokensUsed:tok})}).then(function(r){return r.json()}).then(function(){ closeTestRunModal();toast('Run recorded','success');loadPromptLab(); document.getElementById('pl-run-input').value='';document.getElementById('pl-run-output').value=''; }); }
function renderPromptRuns() { var el=document.getElementById('pl-runs-list');if(!el)return;var f=plCurrentId?plRuns.filter(function(r){return r.templateId===plCurrentId}):plRuns;el.innerHTML=f.length?f.map(function(r){return '<div style="padding:8px;margin-bottom:4px;border:1px solid var(--border);border-radius:4px;font-size:10px;"><div style="display:flex;justify-content:space-between;"><span style="color:var(--text2);font-weight:500;">'+esc(r.model)+(r.variant?' ['+r.variant+']':'')+'</span><span style="color:'+(r.score!=null?(r.score>=0.7?'#4ade80':(r.score>=0.4?'#fbbf24':'#f87171')):'var(--text3)')+'">score:'+(r.score!=null?r.score.toFixed(2):'—')+(r.latencyMs?' · '+r.latencyMs+'ms':'')+(r.tokensUsed?' · '+r.tokensUsed+'t':'')+'</span></div><div style="color:var(--text3);margin-top:2px;">'+timeAgo(r.createdAt)+'</div><div style="color:var(--text2);margin-top:2px;max-height:40px;overflow:hidden;text-overflow:ellipsis;">'+esc(r.input.slice(0,100))+'</div></div>'}).join(''):'<div style="font-size:10px;color:var(--text3);">No runs yet</div>'; }
function generatePLVariations() { if(!plCurrentId)return;var c=document.getElementById('pl-editor-text').value;if(!c)return;fetch(BASE+'/api/prompts/variations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({content:c,count:3})}).then(function(r){return r.json()}).then(function(data){ var v=data.variations||[]; if(!v.length){toast('No variations generated','info');return;} var html='<div style="padding:12px;font-size:11px;font-weight:600;color:var(--text2);">Generated Variations</div>'; v.forEach(function(vr,i){ html+='<div class="card-sm" style="margin:0 12px 8px;padding:10px;"><div style="font-size:10px;color:var(--accent);margin-bottom:4px;">'+esc(vr.expectedStrength)+'</div><pre style="background:var(--bg2);padding:8px;border-radius:4px;font-size:10px;white-space:pre-wrap;max-height:100px;overflow-y:auto;">'+esc(vr.content)+'</pre><div style="font-size:9px;color:var(--text3);margin-top:4px;">'+esc(vr.rationale)+'</div><button class="btn btn-ghost" style="margin-top:6px;font-size:10px;padding:2px 8px;" onclick="applyVariation(\\''+i+'\\')">Apply</button></div>'; }); document.getElementById('pl-runs-list').innerHTML=html; }); }
function applyVariation(idx) { fetch(BASE+'/api/prompts/variations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({content:document.getElementById('pl-editor-text').value,count:3})}).then(function(r){return r.json()}).then(function(data){ var v=data.variations||[]; if(v[idx]){ document.getElementById('pl-editor-text').value=v[idx].content; toast('Variation applied','success'); loadPromptLab(); } }); }
function generatePrompt() { var task=document.getElementById('pl-gen-task').value.trim();if(!task){toast('Task description required','error');return;} var body={task:task,role:document.getElementById('pl-gen-role').value.trim()||undefined,tone:document.getElementById('pl-gen-tone').value||undefined,style:document.getElementById('pl-gen-style').value||undefined,length:document.getElementById('pl-gen-length').value||undefined}; var c=document.getElementById('pl-gen-constraints').value.trim(); if(c) body.constraints=c.split(/\\n/).filter(Boolean); var e=document.getElementById('pl-gen-examples').value.trim(); if(e) body.examples=e.split(/\\n/).filter(Boolean); fetch(BASE+'/api/prompts/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(r){return r.json()}).then(function(data){ plGeneratedPrompt=data.prompt||''; document.getElementById('pl-gen-output').textContent=plGeneratedPrompt; document.getElementById('pl-gen-result').style.display='block'; }); }
function useGeneratedPrompt() { if(!plGeneratedPrompt)return;var n=prompt('Template name:');if(!n)return;fetch(BASE+'/api/prompts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:n,content:plGeneratedPrompt})}).then(function(r){return r.json()}).then(function(){toast('Template created','success');switchPLTab('templates');loadPromptLab();}); }
function copyGeneratedPrompt() { if(!plGeneratedPrompt)return;navigator.clipboard.writeText(plGeneratedPrompt).then(function(){toast('Copied','success')}).catch(function(){ prompt('Copy this prompt:',plGeneratedPrompt); }); }
function loadABTests() { fetch(BASE+'/api/prompts/ab-tests').then(function(r){return r.json()}).then(function(data){ plABTests=data.abTests||[]; renderABTests(); }).catch(function(){}); }
function renderABTests() { var el=document.getElementById('pl-abtests-list');if(!el)return;el.innerHTML=plABTests.length?plABTests.map(function(t){ return '<div class="card-sm" style="cursor:pointer;margin-bottom:4px;padding:8px 12px;'+(plCurrentABId===t.id?'border-left:3px solid var(--accent);':'')+'" onclick="selectABTest(\\''+t.id+'\\')"><div style="font-weight:500;font-size:12px;">'+esc(t.name)+'</div><div style="font-size:10px;color:'+(t.status==='running'?'#4ade80':(t.status==='completed'?'var(--accent2)':'#fbbf24'))+'">'+t.status.toUpperCase()+'</div><div style="font-size:9px;color:var(--text3);">'+timeAgo(t.createdAt)+'</div></div>' }).join(''):'<div class="empty" style="padding:12px;">No experiments yet</div>'; }
function selectABTest(id) { plCurrentABId=id; fetch(BASE+'/api/prompts/ab-tests/'+id).then(function(r){return r.json()}).then(function(t){ var d=document.getElementById('pl-abtest-detail'); var s=t.stats; var w=s&&s.winner; var aRuns=s&&s.variantA?s.variantA.runs:0;var bRuns=s&&s.variantB?s.variantB.runs:0; var aAvg=s&&s.variantA?s.variantA.avgScore.toFixed(2):'--';var bAvg=s&&s.variantB?s.variantB.avgScore.toFixed(2):'--'; var aLat=s&&s.variantA&&s.variantA.avgLatency?s.variantA.avgLatency.toFixed(0)+'ms':'--';var bLat=s&&s.variantB&&s.variantB.avgLatency?s.variantB.avgLatency.toFixed(0)+'ms':'--'; d.innerHTML='<div style="padding:20px;overflow-y:auto;flex:1;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;"><div><div style="font-size:14px;font-weight:600;">'+esc(t.name)+'</div><div style="font-size:10px;color:var(--text3);">Created '+timeAgo(t.createdAt)+'</div></div><div style="display:flex;gap:6px;">'+(t.status==='running'?'<button class="btn btn-ghost" style="font-size:10px;padding:3px 8px;" onclick="plPauseABTest(\\''+t.id+'\\')">Pause</button>':(t.status==='paused'?'<button class="btn btn-ghost" style="font-size:10px;padding:3px 8px;" onclick="plResumeABTest(\\''+t.id+'\\')">Resume</button>':'<button class="btn btn-ghost" style="font-size:10px;padding:3px 8px;" onclick="plResumeABTest(\\''+t.id+'\\')">Restart</button>'))+'<button class="btn btn-ghost" style="font-size:10px;padding:3px 8px;" onclick="plCompleteABTest(\\''+t.id+'\\')">Complete</button></div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;"><div style="border:1px solid var(--border);border-radius:8px;padding:16px;"><div style="font-size:10px;text-transform:uppercase;letter-spacing:0.05em;color:var(--text3);margin-bottom:8px;">Variant A (Control)</div><pre style="background:var(--bg2);padding:12px;border-radius:6px;font-size:11px;white-space:pre-wrap;max-height:150px;overflow-y:auto;font-family:monospace;">'+esc(t.variantA)+'</pre><div style="margin-top:12px;display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:10px;"><div><span style="color:var(--text3);">Runs:</span> '+aRuns+'</div><div><span style="color:var(--text3);">Avg Score:</span> <span style="color:#4ade80;font-weight:600;">'+aAvg+'</span></div><div><span style="color:var(--text3);">Latency:</span> '+aLat+'</div></div></div><div style="border:1px solid var(--border);border-radius:8px;padding:16px;"><div style="font-size:10px;text-transform:uppercase;letter-spacing:0.05em;color:var(--text3);margin-bottom:8px;">Variant B (Experiment)</div><pre style="background:var(--bg2);padding:12px;border-radius:6px;font-size:11px;white-space:pre-wrap;max-height:150px;overflow-y:auto;font-family:monospace;">'+esc(t.variantB)+'</pre><div style="margin-top:12px;display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:10px;"><div><span style="color:var(--text3);">Runs:</span> '+bRuns+'</div><div><span style="color:var(--text3);">Avg Score:</span> <span style="color:#60a5fa;font-weight:600;">'+bAvg+'</span></div><div><span style="color:var(--text3);">Latency:</span> '+bLat+'</div></div></div></div>'+(w?'<div style="margin-top:16px;padding:12px;border-radius:8px;background:var(--bg2);"><div style="font-size:12px;font-weight:600;">'+(w==='tie'?'Tie -- no significant difference':'Winner: Variant <span style="color:'+(w==='A'?'#4ade80':'#60a5fa')+'">'+w+'</span>')+'</div>'+(s.confidence?'<div style="font-size:10px;color:var(--text3);margin-top:2px;">Confidence: '+(s.confidence*100).toFixed(0)+'%</div>':'')+'</div>':'')+'</div>'; }).catch(function(){}); }
function showABTestCreateModal() { var sel=document.getElementById('pl-ab-template'); sel.innerHTML=plTemplates.length?plTemplates.map(function(t){return '<option value="'+t.id+'">'+esc(t.name)+' (v'+t.version+')</option>'}).join(''):'<option value="">No templates available</option>'; document.getElementById('pl-abtest-modal').style.display='flex'; if(plTemplates.length&&plCurrentId){ var t=plTemplates.find(function(x){return x.id===plCurrentId}); if(t){ document.getElementById('pl-ab-variantA').value=t.content; } } }
function closeABTestModal() { document.getElementById('pl-abtest-modal').style.display='none'; }
function createABTest() { var n=document.getElementById('pl-ab-name').value.trim(); var tid=document.getElementById('pl-ab-template').value; var a=document.getElementById('pl-ab-variantA').value.trim(); var b=document.getElementById('pl-ab-variantB').value.trim(); if(!n||!tid||!a||!b){toast('All fields required','error');return;} fetch(BASE+'/api/prompts/ab-tests',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:n,templateId:tid,variantA:a,variantB:b})}).then(function(r){return r.json()}).then(function(){ closeABTestModal();toast('A/B test created','success');switchPLTab('abtests');loadABTests(); }).catch(function(){toast('Failed to create','error')}); }
function updateABTestStatus(id, status) { fetch(BASE+'/api/prompts/ab-tests/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:status})}).then(function(){toast(status.charAt(0).toUpperCase()+status.slice(1),'success');loadABTests();if(plCurrentABId===id) selectABTest(id);}); }
function plPauseABTest(id) { updateABTestStatus(id, 'paused'); }
function plResumeABTest(id) { updateABTestStatus(id, 'running'); }
function plCompleteABTest(id) { updateABTestStatus(id, 'completed'); }
