...
export const routes: RouteHandler[] = [
  ...
  },
  {
    ...
    handler: async (_req, path) => {
      ...
      if (conn && conn.config.transport === 'http') { await disconnectHttp(m[1]); } else { await disconnectStdio(m[1]); }
      return json({ ok: true });
    },
  },
  {
  ...
  },
  {
    ...
    handler: async (_req, path) => {
      ...
      try {
        if (conn.config.transport === 'http') { await connectHttp(conn.config); } else { await connectStdio(conn.config); }
        return json({ ok: true });
      } catch (e) { return err((e as Error).message, 400); }
    },
  },
  ...
  },
  {
    ...
    handler: async () => json({ ok: true, running: true, note: 'MCP server runs in-process — use server restart to stop' }),
  },
  {
    method: 'GET',
    ...
    handler: async () => {
      ...
      const servers = listServers().map((s) => ({ id: s.id, name: s.name, endpoint: s.endpoint, transport: s.transport, status: s.status, toolCount: s.toolCount, lastHealthCheck: s.lastHealthCheck }));
  ...
  },
  {
    ...
    handler: async () => {
      try {
        ...
        return json({ running: !!conn, connected: conn?.connected || false, serverInfo: conn?.serverInfo || null, tools: conn?.tools?.length || 0, calls: conn?.calls || 0, errors: conn?.errors || 0, toolNames: conn?.tools?.map((t) => t.name) || [] });
      } catch {
        return json({ running: false, connected: false, serverInfo: null, tools: 0, calls: 0, errors: 0, toolNames: [] });
      }
    },
  },
  {
  ...
  },
  {
    ...
    handler: async () => {
      try { await (await import('../../mcp/client.ts')).disconnectStdio('chrome-bridge'); return json({ ok: true }); } catch (e) { return err((e as Error).message, 400); }
    },
  },
  {
    method: 'GET',
