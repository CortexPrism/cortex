...
export type NodeMessage =
  ...
  | { type: 'register'; agentId: string; name: string; token: string; capabilities: string[]; version: string; tier?: string; group?: string; lastProcessedDirectiveId?: string }
  ...
  | { type: 'directive'; id: string; sessionId: string; action: string; params: Record<string, unknown>; stream?: boolean; timeoutMs?: number }
  | { type: 'cancel'; directiveId: string }
  | { type: 'config_update'; agentId: string; policies?: Record<string, unknown>; toolsAllowList?: string[] }
