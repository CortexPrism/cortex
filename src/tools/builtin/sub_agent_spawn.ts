...
- write_staged mode is unavailable at depth 2+ (grandchildren and beyond).`,
  ...
  async execute(
    ...
    if (mode === 'write_staged') {
      const { isIsolationAvailable, captureBaseSnapshot } = await import(
    ...
    // Strip orchestration tools from child's tool allow-list to prevent recursive spawning
    ...
    try {
      ...
      // Capture base snapshot for write_staged mode
...
async function autoApplyChangeBundle(
  ...
  const { nanoid } = await import('../../agent/helpers/nanoid.ts');
