...
mcpGatewayCommand
  ...
  .action(async () => {
    const { listServers, getHealthyServers, getDegradedServers, getServerCount } = await import('../mcp-gateway/mod.ts');
    ...
    console.log(bold('\nMCP Gateway Status'));
    console.log(`Total: ${getServerCount()} | ${green(`Healthy: ${healthy.length}`)} | ${degraded.length > 0 ? red(`Degraded: ${degraded.length}`) : `Degraded: 0`}`);
    console.log('');
    ...
    for (const server of servers) {
      const statusColor = server.status === 'healthy' ? green : server.status === 'degraded' ? yellow : red;
      console.log(`  ${bold(server.name)} (${server.id})`);
      console.log(`    Status: ${statusColor(server.status)} | Transport: ${server.transport} | Tools: ${server.toolCount}`);
      console.log(`    Endpoint: ${server.endpoint}`);
      if (server.lastHealthCheck) console.log(`    Last check: ${new Date(server.lastHealthCheck).toLocaleString()}`);
      console.log('');
    }
  });
