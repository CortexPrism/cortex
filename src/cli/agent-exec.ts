...
import { loadAgentIdentity } from '../agent/manager.ts';
...
import { getDefaultAgent } from '../agent/manager.ts';
...
export const execCommand = cortexCommand('exec')
  ...
  .action(async (opts: Record<string, unknown>, ctx: Ctx, prompt: string) => {
    ...
    const agent = agentId ? await (await import('../agent/manager.ts')).getAgent(agentId) : null;
